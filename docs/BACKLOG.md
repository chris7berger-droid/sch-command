...
| ADJ-7 | T3   | Open   | Retire remaining read-only `billing_log` readers       | billing-forecast build (T3, 2026-06-18) | **Repo: sch-command. Deferred follow-up per plan §7** (billing_log kept READ-ONLY, not deleted; full reader retirement is post-proof, reversible). The v1 build stopped ALL `billing_log` writes and rewired the `/jobs` landing cards, but two READ-ONLY consumers still read the now-frozen `billing_log`: **JobDetail billing-history tab** (`JobDetail.jsx:76`) and the **Billing Report export** (`exports.js:110`). They won't crash (table retained) but show frozen legacy percent data. **Also:** the plan's §5.2a 9-site census UNDERSTATED the footprint — `billing_log` was also woven through `StageJobCard` (banner warning + management 'BILLED %' tile), `Jobs.jsx` `urgencyScore`, and `Staged/OnHoldCardList` forwarding; all of those were cleaned in v1. **Fix:** re-point JobDetail history + the export at the invoice/worklist source (or retire), then drop `billing_log` once the new surface is proven. |
