...
- **[DISPUTED]** — successive audit rounds reached opposite conclusions and the repo cannot settle it; stated as an open question with a hard empirical gate, NOT resolved by picking a winner. (Introduced round-2 audit, finding A1 — PowerSync `job_wtcs` tenant isolation.)
...
> - `StageJobCard.jsx` `handleResume()` (On-Hold → 'Scheduled') → `updateJobFields(job.job_id,{status:'Scheduled', ready_confirmed_at:null}, changedBy, 'on_hold_resume', {skipAuditFields:['ready_confirmed_at']})`, **no stage write** — this is the **resume-side stage write** the new approach must add. After routing through `updateJobStatus`, `'Scheduled'` resolves to stage `'Scheduled'` (in-filter). **[DERIVED — verified: `useCallback` at :409, `updateJobFields(...)` call :411–:417, status literal `{ status: 'Scheduled', ... }` at :413. Brief's `:411` = the call's opening line.]**
...
>
> **New approach (hybrid chokepoint):** introduce a single **`updateJobStatus(jobId, newStatus, changedBy, source?)`** helper in `queries.js` that (1) writes `jobs.status` via the existing audit-logged `updateJobField` path, then (2) **unconditionally** resolves and writes the paired `call_log.stage` from a **complete, fully-enumerated `STATUS_TO_STAGE` map** (`Scheduled→Scheduled, In Progress→In Progress, On Hold→In Progress, Ongoing→In Progress, Complete→Complete` — see SCH3), and **throws on any unmapped status (fail-closed)**. **Route ALL three writers above through it** (`JobCardList:102`, `StageJobCard:403`, `StageJobCard` `handleResume` :413) — delete their ad-hoc `stageMap`/missing-sync code. Because stage resolution lives **inside** the helper, a future writer that calls `updateJobStatus` cannot forget to sync; because it throws on unmapped, a new status can't silently skip the stage write. Pair it with the grep gate (SCH3) so no raw `from('jobs').update({status})` / `updateJobField(_, 'status')` bypasses it.
>
...
>
> **No sync-rule change needed [LOCKED 2026-06-12 — Option 1, kills round-2 A2]:** On Hold maps to `'In Progress'`, which is **already** in the Field `call_log.stage` filter (`Scheduled / In Progress / Parked / mobilized / in_progress` — §3.5). So **no `powersync-sync-rules.yaml` filter edit is required** for the On-Hold path; SCH3 collapses to a `queries.js`-only change. This supersedes the earlier "sync-rule change in the same step" coupling. (The original amendment text above is retained for history but the lockstep filter edit it called for is no longer needed.)
...
- **[LOCKED — round-2 A4: `dates_tbd` is read PER-WTC, not as one proposal-wide scalar].** `dates_tbd` lives on `proposal_wtc` (§6.2) — it is a **per-WTC** column, so the send loop must read **`wtc.dates_tbd`** for each WTC, not a single proposal-level flag. A proposal can carry two WTCs where one has known dates and the other is TBD; collapsing them to one scalar would either reject the dated WTC or wrongly null its dates. Each `job_wtcs` row inherits **its own** WTC's `dates_tbd` to decide whether `start_date`/`end_date` (and the seeded per-day `field_sow[*].date`) are null. The toggle is **persisted to `proposal_wtc.dates_tbd`** at sale (§S2) and **read back per-WTC at Send-to-Schedule** — there is no single proposal-wide TBD state.
- **[LOCKED L1 — NULL-safety on the "dates TBD" path]** Explicitly write `field_sow: wtc.field_sow || []` (the `[]` literal, **never `undefined`**) and `start_date`/`end_date` as `null` when **that WTC's** `wtc.dates_tbd` is on. `job_wtcs.field_sow` stays `NOT NULL`, so an `undefined` would violate it; `job_wtcs.start_date`/`end_date` are being **dropped to nullable** (see §6.6 migration) precisely so a "dates TBD" row can insert with null dates. Without that migration the create table's `start_date NOT NULL`/`end_date NOT NULL` (`20260512120100_job_wtcs_create.sql`) would reject every TBD send. **[DERIVED: §3.3 line 71 confirms both columns are `date` NOT NULL in the create migration.]**
...
- **[DERIVED — new audit-logged writer in `queries.js`]** Persist via a new **`updateJobWtcFieldSow(jobWtcId, nextFieldSow, changedBy, source = 'schedule_command')`** in `queries.js`, mirroring the existing `updateJobField`/`updateJobFields` audit-logging convention **[verified pattern: queries.js:155-188 reads-old → writes → inserts a `job_changes` row]**. It updates `job_wtcs.field_sow` (and `start_date`/`end_date` when the WTC span shifts) and writes a `job_changes` audit row keyed on the parent job's `call_log_id`. **Do not** write `job_wtcs` via a raw `supabase.from('job_wtcs').update(...)` — all Schedule per-day-date / `field_sow` writes route through this helper so the change is audit-logged like every other job write (`queries.js` is the audit chokepoint per `CLAUDE.md` "All job writes go through `updateJobField()` / `updateJobFields()`").
  ...
  - **Serialize old/new values with `JSON.stringify`, NEVER `String()` [LOCKED — round-2 A3].** `field_sow` is a JS **array of day objects**, not a scalar. The existing scalar helpers serialize with `String(...)`: `updateJobField` does `String(current[field] ?? '')` / `String(newValue ?? '')` and `updateJobFields` does `String(current?.[field] ?? '')` / `String(updates[field] ?? '')`. **[DERIVED — verified `queries.js:163-164` (updateJobField) and `:214-215` (updateJobFields).]** `String()` on an array/object yields **`"[object Object]"`** (or a comma-joined, structure-losing string for an array of objects) — the `job_changes` audit row would record garbage and the history view would be useless. **`updateJobWtcFieldSow` must serialize the old + new `field_sow` (and any `start_date`/`end_date`) with `JSON.stringify(value)`** before writing `old_value` / `new_value`. This is a **correction**, not a match, of the existing convention: the scalar helpers' `String()` is fine for their scalar fields (`status`, `start_date`, dates, numbers) but is **wrong for the `field_sow` JSONB blob** this helper writes. Guard the parse side too (history reader should `JSON.parse` defensively, since legacy rows may hold non-JSON `String()` output — but those are pre-vertical `jobs.field_sow` writes, not `job_wtc.field_sow` rows, which only this helper creates).
...
- **The complete `STATUS_TO_STAGE` literal [LOCKED 2026-06-12 — R-A3]** — fully enumerated, no omissions:
  ```js
  const STATUS_TO_STAGE = {
    'Scheduled':   'Scheduled',
    'In Progress': 'In Progress',
    'On Hold':     'In Progress',   // Option 1 (LOCKED 2026-06-12) — see below
    'Ongoing':     'In Progress',
    'Complete':    'Complete',
  }
  ```
- **`updateJobStatus()` THROWS on any unmapped status (fail-closed) [LOCKED — R-A3].** If `STATUS_TO_STAGE[newStatus]` is `undefined`, the helper **throws** (does not silently skip the `call_log.stage` write, and does not write `jobs.status` either — fail before the first write, or roll back). This is the structural guarantee that replaces "remember to also sync": a new status that nobody mapped surfaces as a hard error at the chokepoint, not as a silently-stale stage that drops the job from crew. **Rationale:** silent-skip is exactly the round-1 failure mode (the partial `stageMap` at `JobCardList:105` omitted On Hold and the job went stale); fail-closed converts that class of bug into a loud, test-catchable throw.
- **Assert/test requirement — `STATUS_TO_STAGE` keys ⊇ `STATUS_OPTIONS_PICKER` [LOCKED — R-A3].** Add a unit assertion (or a startup invariant) that **every** value in `STATUS_OPTIONS_PICKER` is a key in `STATUS_TO_STAGE`. **[DERIVED — verified location: `STATUS_OPTIONS_PICKER` is at `src/lib/jobStatus.js:21` (brief correct), and its current contents are `['Scheduled', 'In Progress', 'On Hold', 'Complete']` (4 values, no 'Ongoing').]** The literal above satisfies the superset: it covers all 4 picker values plus the legacy-readable `'Ongoing'` and `'Complete'`. The assert is what guarantees a future dropdown addition can't slip past the throw — if someone adds a picker option without a map entry, the test fails before ship.
- **Route ALL real `jobs.status` update-path writers through it [DERIVED — verified complete set]:**
  ...
  - `StageJobCard.jsx` `handleResume` (On-Hold → 'Scheduled') — currently `updateJobFields(... {status:'Scheduled', ready_confirmed_at:null})` with **no stage sync**; this is the **resume-side stage write**. Route the status part through `updateJobStatus` (keep the `ready_confirmed_at: null` + `skipAuditFields` handling — either fold into the helper's signature or write `ready_confirmed_at` separately so the existing `on_hold_resume` audit source is preserved). **Confirm resume writes `status='Scheduled'` → `STATUS_TO_STAGE['Scheduled'] = 'Scheduled'`**, so a resumed job lands on the `Scheduled` stage (in-filter). **[DERIVED — verified: `handleResume` is the `useCallback` at `StageJobCard.jsx:409`; the `updateJobFields(...)` call spans :411–:417 with the literal `{ status: 'Scheduled', ready_confirmed_at: null }` on :413 and `source 'on_hold_resume'` + `skipAuditFields:['ready_confirmed_at']` on :414–:415. The brief's `:411` points at the call's opening line; the status literal itself is `:413`.]**
...
- **On Hold → `'In Progress'` — DESIGN CALL = Option 1 [LOCKED by Chris 2026-06-12].** 'On Hold' maps to the stage `'In Progress'`. **`'In Progress'` is already in the Field `call_log.stage` sync filter** (`Scheduled / In Progress / Parked / mobilized / in_progress` — §3.5), so a held job stays synced to the crew with **no PowerSync filter edit required**. This is consistent with the locked "On Hold retains crew visibility": a held job keeps its prior in-filter stage rather than acquiring a new out-of-filter `'On Hold'` stage that would drop it. **This LOCK kills round-2 finding A2** (the proposed sync-rule filter edit) — there is nothing to edit on `powersync-sync-rules.yaml` for the On-Hold path. The map's `'On Hold': 'In Progress'` entry is the entire fix; the helper writes it unconditionally and throws on anything unmapped (above).
- **No sync-rule edit for the On-Hold path [LOCKED — Option 1].** Because On Hold reuses the in-filter `'In Progress'` stage, `field-command/powersync-sync-rules.yaml`'s `call_log.stage` filter is **unchanged by SCH3**. (F1 still adds the `job_wtcs` bucket — that is a separate sync-rule change in §7, not this one.) This removes the round-1 "[AMENDMENT] sync-rule edit in the same step" coupling: SCH3 is now a `queries.js`-only change.
...
- **RLS note [DISPUTED — round-2 A1]:** Field is single-tenant today (HDSP only). The `job_wtcs` authenticated RLS policies are scoped via the parent `jobs → call_log.tenant_id` chain — **`job_wtcs` itself has no `tenant_id` column**; it reaches a tenant only through `jobs.call_log_id → call_log.tenant_id`. **[DERIVED — verified `20260512120100_job_wtcs_create.sql`: no `tenant_id` column on the table; the `job_wtcs_select_authenticated` policy reads `EXISTS (SELECT 1 FROM jobs j JOIN call_log cl ON cl.id = j.call_log_id WHERE j.job_id = job_wtcs.job_id AND cl.tenant_id = get_user_tenant_id())`. The migration's own comment states "jobs itself has no tenant_id column."]**
  - **OPEN QUESTION — does PowerSync enforce tenant isolation on the `job_wtcs` bucket? [DISPUTED]** Round 1 and round 2 reached **opposite conclusions** on whether PowerSync's sync connection respects (or bypasses) the `jobs → call_log.tenant_id` RLS chain, and **this cannot be settled from the repo** (it depends on the deployed PowerSync sync-rule auth model and connector config, which live in the PowerSync dashboard, not in version-controlled files). **Do NOT assume either way.** The earlier "PowerSync syncs with a service-level connection" assertion is **withdrawn** — whether the sync role is service-level (RLS-bypassing) or token-scoped (RLS-honoring) is the open question, not a settled fact. Treat tenant isolation on this bucket as **unverified** until the gate below passes.
  - **HARD GATE before onboarding a 2nd customer — two-tenant sync test [DISPUTED→must be empirically settled].** Before any second tenant is onboarded, run a live two-tenant sync test: create a `job_wtcs` row reachable only by **tenant B** (via a tenant-B `jobs → call_log`), and confirm it **does NOT sync to a tenant-A device**. If tenant B's `job_wtcs` row appears on tenant A's phone, PowerSync is **not** enforcing the `call_log.tenant_id` chain for this bucket and a sync-rule-level tenant filter (or per-tenant bucket parameter) must be added before multi-tenant ships. This gate is **required** — it converts the disputed, repo-unsettleable question into an observed result. (Consistent with the §11 deployment context "1 live tenant — multi-tenant onboarding not shipped": there is time to gate this before the 2nd customer, but it must not be skipped.)
...
- **Precise merge spec [DERIVED — per round-1 audit].** After F2 gathers all WTCs' `field_sow` arrays, merge the days as follows:
  ...
  2. **Within a date group, across the work types that land that day:**
     - **CONCAT** `tasks` (append all tasks from every contributing day — work types collapsed, not broken out). **[round-2 A5] Keep a work-type tag per task** (the contributing WTC's `work_type_name`) as **metadata on each task** so the merged task list still carries its trade label. This does **NOT** violate the locked "day-centric, work types collapsed" view (L4): the **grouping stays by date** and the rendered view is still one day-centric list — the tag is just a per-task label, not a separate per-work-type section/pill. (It's also what makes the MAX-vs-SUM crew rule below computable: you need to know which tasks belong to which work type to take a max across work types.)
     - **CONCAT** `materials` (append all materials).
     - **`crew_count` = MAX across work types that day, NOT SUM [round-2 A5].** Two work types on the same calendar date typically share the **same** crew (the crew does multiple trades that day) — **summing double-counts** them (e.g. WTC-A says 4 crew, WTC-B says 4 crew, but it's the *same* 4 people → the day needs 4, not 8). Take the **max** of the contributing days' `crew_count` as the day's crew figure. **⚠ CONFIRM WITH THE FIELD MANAGER (Jonah)** — whether same-day work types always share crew (MAX) or can be genuinely additive (distinct crews → SUM) is a **real-world operational question, not settled here**. Flag MAX as the working assumption pending Jonah's confirmation; the per-task work-type tag (above) keeps both computable, so the rule can flip without re-architecting the merge.
     - **SUM** `hours_planned` (total planned man-hours that day — hours are additive even when crew is shared, since each task still consumes its own labor time). _(If Jonah's answer changes the crew model, revisit whether hours should also change — but hours summing is the safer default.)_
...
1. **SCH3 — `updateJobStatus()` chokepoint** (§5, per the §3.6 amendment). A **`queries.js`-only** change now (Option 1 locked 2026-06-12 removed the coupled filter edit): the `updateJobStatus` helper with the complete `STATUS_TO_STAGE` map (`On Hold → In Progress`, throws on unmapped) and all 3 writers (`JobCardList:102`, `StageJobCard:403` kickoff, `StageJobCard:413` resume) routed through it. **No `powersync-sync-rules.yaml` filter edit** — On Hold reuses the in-filter `'In Progress'` stage, so a held job stays synced without any sync-rule deploy. Land + verify before any dated SOW is expected on a phone. **[PREREQUISITE]**
...
|---|------|----------|-------------|
| R1 | **Stage-sync hard-break** — `jobs.status` writes that don't sync `call_log.stage` drop the job from Field's sync filter mid-lifecycle. | High (crew can't see the job) | **SCH3** — `updateJobStatus()` chokepoint (all 3 verified writers routed through it) + the now-**complete** `STATUS_TO_STAGE` map (`Scheduled→Scheduled, In Progress→In Progress, On Hold→In Progress, Ongoing→In Progress, Complete→Complete`) that **throws on any unmapped status (fail-closed)** + a `keys ⊇ STATUS_OPTIONS_PICKER` assert. **No sync-filter edit** — On Hold maps to the already-in-filter `'In Progress'` (Option 1, LOCKED 2026-06-12; kills round-2 A2). `queries.js`-only. PREREQUISITE, ships first. |
...
| R7 | **PowerSync auth vs. `job_wtcs` RLS [DISPUTED — A1]** — two coupled unknowns: (a) the sync role must let `job_wtcs` rows through (else Field syncs empty), and (b) **whether PowerSync enforces tenant isolation** on this bucket (the `jobs→call_log.tenant_id` chain) is an **OPEN question round 1↔2 disagreed on and the repo can't settle** — `job_wtcs` has no `tenant_id` column, reaching a tenant only via `jobs→call_log`. | Med | (a) F1 verification — confirm rows return under deployed PowerSync auth before shipping F2. (b) **HARD GATE (§F1): a two-tenant sync test** — tenant B's `job_wtcs` row must NOT sync to a tenant-A device — **required before onboarding a 2nd customer**. Do not assume isolation either way. |
...
| R9 | **Crew-count merge assumption (A5)** — F3 merges same-date work types with **MAX `crew_count`** (shared-crew assumption). If two same-day work types are genuinely staffed by **distinct crews**, MAX under-reports the day's crew need. | Med (wrong crew figure shown to field) | **F3 uses MAX as the working rule + keeps a per-task work-type tag** so the rule is reversible. **CONFIRM the MAX-vs-SUM rule + per-task tag with field manager (Jonah)** — real-world operational confirmation, NOT settled in-plan. |

---

## §11 Audit manifest

_Generated by `/auditcriteria` on 2026-06-12 (round 3). Consumed by `/runaudit` to size the adversarial audit pass._
...
### Round
- Current round: 3
- Plan revision under audit: `ed03f5f`
- Findings trend: round 1 (6: 1H/5M, structural) → round 2 (6 main / 13 deduped: 1H/8M/4L, spec-precision + 1 regression) → round 3 (?) — **CONVERGING** (severity falling, no scope growth); not a plateau.

### Prior rounds
- Round 1: `1a7f2d0` · 1H/5M (6 findings) · pattern: `writer-coverage`
- Round 2: `ed03f5f` · 1H/8M/4L (13 deduped) · pattern: `stage-map-completeness`

**Briefing for agents**: do NOT re-find round-1 or round-2 issues. Round-2 fixes (commit `ed03f5f`): R-A3 completed the `STATUS_TO_STAGE` map (On Hold→In Progress, **Option 1 — no PowerSync filter edit**) + throw-on-unmapped + keys⊇picker assert; A1 marked the PowerSync RLS premise **[DISPUTED]** with a two-tenant sync test as a hard pre-multi-tenant gate; A3 `updateJobWtcFieldSow` `JSON.stringify` (not `String()`) for the audit row; A4 per-WTC `dates_tbd` read at send; A5 F3 `crew_count`=MAX (pending Jonah confirm) + per-task work-type tag. Overage stays **CUT to Build 2** — do NOT audit it. Attack ONLY material new to `ed03f5f` + REGRESSION-check the round-2 fixes landed. **NOTE: the [DISPUTED] RLS premise is intentionally unresolved (an empirical 2-tenant test gates it) — do not re-litigate it as a finding; just verify the gate is specified.**
...
- Total lines: 456
- Sections: 12
- [LOCKED] decisions: 37 tags · [AMENDMENT]: 4 · [DISPUTED]: 5 (the PowerSync RLS premise)
- [DESIGN-OPEN] items: 0 (comparand moved to Build-2 seed; no new opens in round 2)
- [OPEN]/[BLOCKED]: R6 ledger BLOCKED-until-repaired; A5 MAX-vs-SUM pending Jonah confirm
- Plan-to-code ratio: ~456 plan : est ~300–450 code across 3 repos ≈ ~1:1 (healthy)
...
- New helpers: `updateJobStatus()` stage-sync chokepoint (+ complete `STATUS_TO_STAGE` map, throw-on-unmapped; On Hold→In Progress means **no** PowerSync filter edit); `updateJobWtcFieldSow()` audit-logged writer
...
### Known weak points
- **R1/SCH3 stage-sync (was High; map now complete in round 2):** all `jobs.status` writes route through `updateJobStatus()` with a complete `STATUS_TO_STAGE` map + throw-on-unmapped + a keys⊇picker assert; On Hold→In Progress keeps the job in-filter with **no** PowerSync filter edit. VERIFY the round-2 fix landed: writer set complete (`JobCardList:102`, `StageJobCard:403/411`; `App.jsx:160` correctly EXCLUDED), the assert exists, resume writes `'Scheduled'`. One bypassing writer → job drops from the crew mid-life.
...
### Open questions
- Count: 0 design-open. Round 3 is a convergence pass — regression-verify round-2 fixes + any residual on the trimmed plan.
- Highest-pressure: did the round-2 stage-map completion + `JSON.stringify` audit serialization + per-WTC `dates_tbd` actually land; is the [DISPUTED] RLS two-tenant gate specified.

### Suggested attack angles (2 total)
1. **Round-2 regression + data/state** — covers Data layer, State model, migrations, write paths. Reading: sch-command `queries.js` (`updateJobStatus`/`STATUS_TO_STAGE`/`updateJobWtcFieldSow`), `jobStatus.js`, `StageJobCard`, `ProposalDetail` send (per-WTC `dates_tbd`), `FieldSowBuilder`. Pressure: did the round-2 fixes land — complete `STATUS_TO_STAGE` map + throw-on-unmapped + keys⊇picker assert; `JSON.stringify` audit serialization (not `String()`); per-WTC `dates_tbd` read; `field_sow: []` empty-write + NOT-NULL drop? Any NEW gap the round-2 edits introduced?
2. **RLS / PowerSync sync + framework-fit** — covers RLS, sync, data layer, audit logging. Reading: `powersync-sync-rules.yaml`, field-command `schema.js`/`TasksTab.js`, `job_wtcs` RLS, sch-command `queries.js`. Pressure: is the [DISPUTED] RLS premise's two-tenant gate properly specified (verify the gate; do NOT re-litigate the dispute)? Field local-`id` read; F3 merge `crew_count`=MAX correctness; new writes through audit-logged helpers; `Jobs.jsx` realtime coverage of `job_wtcs`.

### Suggested agent count: 2

Rationale: round 3 is a convergence pass — rounds 1–2 cleared the structural + spec gaps and the only High was a now-fixed regression; 2 focused agents (regression-verify + RLS/sync) suffice. Bump to 3 only if round 3 surfaces material new gaps.
