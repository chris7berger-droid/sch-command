...
- In the Schedule Field SOW editor (§SCH1/SCH2): on a save that adds a day/material whose cost exceeds the frozen bid, set `jobs.has_overage = true` and insert a `job_overages` row. **Do not block the save** (L5: capture, don't gate) and **do not write `proposal_wtc`** (L2). **[LOCKED 2026-06-11 — Chris ratified]** Cost-comparison basis: **committed `job_wtcs` cost vs. `proposals.total`, accrual-style** (counted when work is committed, not paid — mirrors the QB accrual rule). Stamp when committed cost > proposal total.
...
_Generated by `/auditcriteria` on 2026-06-11. Consumed by `/runaudit` to size the adversarial audit pass._

### Bottom line (plain English)
A big, cross-app plan that finishes the SOW pipeline across Sales, Schedule, and Field. It is **money-observant, not money-touching**: the only money interaction is a *read-only comparison* — original proposal cost vs. actual committed cost — that **flags** when an SOW grows past the original scope. It writes an observation (a `has_overage` flag + a `job_overages` record), never money, and the frozen-bid rule guarantees nothing downstream can change the bid. It still deserves a real review — four reviewers, each on a different risk: the write path + the "bid stays frozen" rule, the database changes, the offline-sync + access rules, and repo-pattern fit. Worth the check before any code is written.

### Round
- Current round: 1
- Plan revision under audit: `963ba50`
- Findings trend: n/a — round 1

### Prior rounds
none — this is round 1. (Agents: attack revision `963ba50`; no prior findings to skip.)

### Deployment context
- **Live tenants**: 1 — HDSP only; multi-tenant onboarding not shipped (blocked, sales-command F7).
- **Prod / staging / dev**: Sales + Schedule Command live in prod; Field Command in early/limited crew use (offline mobile). The `job_wtcs` read side is merged to `main` but **dormant** (no writer) — the *write* path this plan adds is net-new to prod.
- **Blocking feature flags**: none gating this surface; multi-tenant blocked.
- **Concurrency profile**: ≤5 — office staff on web + field crew on per-device offline phones. No high-concurrency write path.

Severity caps: cross-tenant findings → **Med** (live_tenants==1); multi-user race findings → **Low** (≤5 / offline-per-device); theoretical multi-tenant attacks → **Low**. **Overage logic is money-observant (read-only): it compares the frozen bid against committed cost and writes only an observation (flag + record) — it never writes money. Cap overage findings at "wrong/missing flag," NOT "financial corruption."**

### Time budget + finding cap
- **Time budget**: not ERD-locked (design→plan; no `/erd-start` yet). Treated as a focused plan audit.
- **Finding cap**: **6** (top-6 caused-by findings in the main report; remainder quarantined). Raise if Chris sets a larger ERD budget.

### Surface
- Total lines: 291
- Sections: 12
- [LOCKED] decisions: 27 tags (L1–L6 design invariants + 8 plan-decision locks ratified 2026-06-11)
- [DESIGN-OPEN] items: 0 (all resolved pre-audit)
- [OPEN]/[BLOCKED] items: 3 (R6 ledger BLOCKED-until-repaired; 2 verification residuals)
- Plan-to-code ratio: ~291 plan : est ~300–500 code across 3 repos ≈ 1:1–1:2 (healthy; not scope-crept)

### Layers touched
- UI / components (WTC date picker, `FieldSowBuilder` day render, JobDetail surfaces, Field `TasksTab`)
- Data layer (`queries.js` loaders/writers, `ProposalDetail` send path)
- State model (new columns + per-day `date` inside `field_sow` JSONB)
- Migrations / schema (`dates_tbd`, `job_overages`, `has_overage`)
- RLS / multi-tenancy (`job_overages` policies; `job_wtcs` RLS vs PowerSync sync role)
- Real-time / sync (PowerSync: add `job_wtcs` to Field sync; the `call_log.stage` sync filter)
- Cross-repo (sales-command writer · sch-command calendar owner · field-command reader, one shared DB)

### New mechanisms introduced
- New columns: `proposal_wtc.dates_tbd` (bool), `jobs.has_overage` (bool), `field_sow[*].date` (JSONB field)
- New table: `job_overages` (+ 4 RLS policies)
- New write path: Sales writes canonical `job_wtcs` rows at Send-to-Schedule (the dormant read side's missing writer)
- New logic: stage-sync choke-point (every `jobs.status` write → `call_log.stage`); overage trip (committed `job_wtcs` cost vs `proposals.total`, accrual); durable `crypto.randomUUID()` day/task IDs
...
- Migration ledger is cross-repo coordinated (O7 unresolved; `db push` broken here)

### Irreversibility
- Migrations: all additive (`dates_tbd`, `job_overages`, `has_overage`) — but cross-repo ledger-coordinated, dashboard-applied + repair (`db push` aborts here; RESUME ALERT)
- No backfill (ratified: fallback only)
- No destructive changes; no public-API change

### Known weak points
- **R1/SCH3 stage-sync hard-break (High):** if any `jobs.status` write bypasses the choke-point, the job silently drops from Field's sync filter mid-lifecycle — crew can't see it. Load-bearing prerequisite.
- **R2 frozen-bid invariant (High):** any Schedule/Field write leaking into `proposal_wtc` or a financial field corrupts the bid. The plan relies on discipline (edit `job_wtcs` only) — verify no write path violates it.
- **R7 PowerSync auth vs `job_wtcs` RLS (Med):** if the sync role doesn't satisfy `jobs→call_log.tenant_id`, `job_wtcs` syncs empty and Field shows nothing; single-tenant Field has no local `tenant_id`.
- **§8.3 overage trip (Med):** money-observant only — *compares* committed cost vs the frozen proposal total and writes one observation (`has_overage` flag + `job_overages` row). Must not block the save and must not write `proposal_wtc`. Worst-case failure = a **wrong/missing flag** (mis-observation), not financial corruption. Verify it matches sales-command "hiding a field is not guarding the save" + "fail-safe not silent" rules.
- **S3 re-send idempotency:** writing `job_wtcs` must upsert-or-skip on `proposal_wtc_id` UNIQUE (mirror existing `23505` handling) or a re-send errors/dupes.
- **Premise:** "`job_wtcs` read side is dormant — nobody writes it." Load-bearing; if a writer exists, S3 is partly redundant.
- **JSONB coercion:** `field_sow[*].date` must not break `parseFloat` coercion at `WTCCalculator.jsx:864` / `FieldSowBuilder`.
- **Framework fit:** overage write + stage-sync must route through `queries.js` audit-logged helpers (`updateJobField`/`updateJobFields`), not raw `.update()`; `Jobs.jsx` realtime doesn't subscribe to `job_wtcs`.
- **R6 ledger drift (Med, BLOCKED):** three live-but-ledger-absent migrations abort `db push`; new migrations dashboard-applied + repaired.

### Open questions
- Count: 0 design-open (all ratified). Residual verification items only, folded into Known weak points + Load-bearing claims.
- Highest-pressure: stage-sync coverage (R1) and the frozen-bid invariant (R2).

### Suggested attack angles (4 total)
1. **User-path state trace + frozen-bid invariant** — covers Data layer, State model, write paths. Reading: sales-command `ProposalDetail.jsx` (send), `WTCCalculator.jsx`; sch-command `queries.js`, `FieldSowBuilder`, §8.3 overage trip, SCH3 stage-sync. Pressure: trace author→send→Schedule-edit→Field; does any write leak into `proposal_wtc` (R2)? Does the stage-sync choke-point fire on **every** real `jobs.status` change path (R1)? Does the overage trip fire correctly and non-blocking?
2. **Cross-app data model / migration / canonical move** — covers Migrations, State model, schema. Reading: `20260512120100_job_wtcs_create.sql`, the `job_overages`/`dates_tbd`/`has_overage` specs, `ProposalDetail` send, sch-command CLAUDE.md RESUME ALERT. Pressure: the dormant-read-side premise; re-send idempotency (upsert on `proposal_wtc_id`); `field_sow[*].date` shape vs coercion; model (a) one-jobs-row holds; ledger-drift deploy abort.
3. **RLS / multi-tenancy / PowerSync sync** — covers RLS, sync, cross-repo bypass. Reading: `powersync-sync-rules.yaml`, field-command `schema.js`/`connector.js`, `job_wtcs` + `job_overages` RLS. Pressure: does the sync role satisfy `job_wtcs` RLS (R7)? Field offline `tenant_id` absence; the `call_log.stage` sync-filter break (R1 from the sync side); `job_overages` RLS correctness; which tables Field actually syncs.
4. **Data-layer / loader correctness + framework fit** — covers Data layer, UI, audit logging. Reading: `queries.js` (loaders, `updateJobField`/`updateJobFields`, realtime sub), `ScheduledCardList`/`jobCardLabel`, Field `TasksTab` day-grouping. Pressure: do the new writes route through the audit-logged `queries.js` chokepoint? Does `Jobs.jsx` realtime cover `job_wtcs`? day-grouped render correctness; the unjoined Field fallback (R3); `withWTCs` hydration coverage.

### Suggested agent count: 4

Rationale: raw formula yields 9 (7 layers + cross-system + ≥3 novel mechanisms), capped at 5; the honest grouping is 4 strong angles (UI folds into framework-fit + user-path). 4 deep beats 5 padded — bump to 5 only to split RLS from PowerSync-sync.

---

### Load-bearing claims to verify (pressure points for agents)

_Each is a riskiest assumption a reviewer should independently verify, with a file pointer._
...
4. **[LOCKED 2026-06-11 — Chris ratified model (a)]** One `jobs` row per proposal + N `job_wtcs` children; `idx_jobs_source_proposal_id` stays (no drop). The Jobs-IA per-WTC-rows lean is superseded. Residual verification only: confirm no code path inserts >1 `jobs` row per proposal. **[Pointer: §6.4; ProposalDetail.jsx:580]**
...
7. **"Overage capture must NOT block the save and must NOT touch `proposal_wtc`."** Verify the §8.3 trip path against the sales-command "hiding a field is not guarding the save" and "fail safe not silent" rules. Comparand now LOCKED (committed `job_wtcs` cost vs `proposals.total`, accrual) — verify the implementation matches that basis and that the non-blocking behavior holds. **[Pointer: §8.3; sales-command/CLAUDE.md Data Integrity Rules 6-7]**
