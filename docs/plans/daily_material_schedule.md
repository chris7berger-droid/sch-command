...
- **Stamped ≠ confirmed — the amber gate, tri-state (grandfather ratified 2026-07-14):**
  ...
  - `specs_confirmed: false` → amber chip, **blocks Send**: *"Specs pulled from Material Memory — confirm for this job's conditions."* **Initialization rule (revision pass 3, F1 — stated once here, referenced by every stamp spec):** a new stamp initializes `false` **only when the source catalog row carries ≥1 non-empty spec value at stamp time**; a no-spec row (most of the ~160 seeds until specs are entered) **omits the key** (absent = exempt — the same grandfather rule). Blank data never demands confirmation; that's the click-through ritual this section forbids.
...
**Legacy, scheduled to retire — WTC-backed paths only; the flat column stays for zero-WTC jobs (Tier 4, step 9):**
...
**Schedule's one-hop add** (`FieldSowBuilder.jsx:94-104` `addCatalogMaterialToDay` — picks straight from the catalog, no Tab-3 line): stamps `catalog_id` + specs + `specs_stamped_at = now()` directly; `specs_confirmed` per the §2 F1 initialization rule (`false` only if the row carries specs; omitted otherwise). (Revision pass 2 — this path previously kept no catalog link by design; that design predates decision #2.)
