...
_Generated by `/auditcriteria` on 2026-07-14 (round 2). Consumed by `/runaudit` to size the adversarial audit pass._

### Bottom line (plain English)
Round 2 is a verification pass, not a re-review: round 1 found the plan's snapshot of today's code was wrong in six places; all six are now fixed against the cited source. Two reviewers confirm the corrections hold and attack only the mechanisms the revision ADDED (the fork-on-edit path, the date-normalized trigger, the tri-state gate) — not the whole doc again.

### Round
- Plan type: feature
- Current round: 2
- Plan revision under audit: 8864e98
- Findings trend: round 1 (6 top + 3 over-cap) → 2 (?) — plateau watch active per §audit rules

### Prior rounds
- Round 1: 5877539 · 4H/2M top-6 + 3 over-cap + 3 adjacent · pattern: wrong-premise-baseline
  - Response (8864e98): A1 reader/writer map + Tier-4 preconditions; A2 Phase-3 retrofit respec; B1 catalog_id/mapping/coercion; C1 grandfather tri-state + print gate (ratified); D1 count-based badge + trigger writer + once-only invariant; E1 re-verified — **RLS half REFUTED against source** (`role_aware_money_rls.sql:132-165` already gates the catalog; SEC-3 adjacent withdrawn), fork-on-edit + silent-no-op kept. All 3 over-cap folded. Adjacent → SEC-2/SEC-4.

**Briefing for agents**: do NOT re-find round-1 issues — the revision-pass commit (8864e98) is the canonical record. Attack ONLY material new to 8864e98. NOTE: round-1's E1 was itself half-wrong; if you assert a policy/code fact, cite the file:line you read this round.

**Plateau signal**: if round-2 count ≥ round-1's, `/runaudit` MUST present scope-cut as the only build-prompt option.

### Deployment context
- **Live tenants**: 1 — HDSP only; multi-tenant onboarding blocked
- **Prod / staging / dev**: shipped SOW/mob pipeline live in prod; everything this plan specs is unbuilt (paper only this loop)
- **Blocking feature flags**: none
- **Concurrency profile**: ≤5 users (office staff), solo in Sales authoring

Cross-tenant findings cap at Med while `live_tenants == 1`. Multi-user race findings cap at Low while ≤5. Theoretical attacks against state that doesn't exist yet are not High.

### Time budget + finding cap
- **Time budget**: 45 min (verification round — smaller than round 1's 60)
- **Finding cap**: 5 findings

### Surface
- Total lines: 310 (was 279 at round 1)
- Sections: 8 (§0 Baseline, §0 why, §1–§6)
- [LOCKED] decisions: 2 (+3 UI decisions carried); C1 grandfather ratified 2026-07-14
- [DESIGN-OPEN] items: 0
- [OPEN] items: 4 (§6)
- Plan-to-code ratio: n/a this loop (paper only); Phase-1 spec ≈ 2 migrations + 1 trigger + jsonb conventions

### Layers touched
(unchanged from round 1: state model, migrations/schema, RLS/auth, cross-repo, PowerSync — but round 2 attacks only the deltas)

### New mechanisms introduced IN THIS REVISION (the round-2 attack surface)
- Tri-state `specs_confirmed` semantics (absent/false/true) + reconfirm-on-edit + Phase-4 print gate as post-Send enforcement (§2)
- `catalog_id` + `specs_stamped_at` across the two-hop stamp + field-mapping table (§4.2)
- Text-typing of `mils`/`mix_time` + coercion-list changes in two components (§4.2)
- Date-normalized `field_sow` diff in the §4.3 revision-stamp trigger (strip per-day `date` before OLD/NEW compare)
- Badge = `sow_revision_count > 0` (§1/§4.3)
- Fork-on-spec-edit for NULL-tenant catalog defaults + "0 rows updated" error surfacing (§4.1)
- Tier-4 retirement preconditions (ReportTab migration, zero-WTC carrier) (§1)
- Once-only-send named invariant (§1/§4.3)

### Cross-system reach
- unchanged (4 repos, shared DB, PowerSync); §4.4 now correctly states client-schema column-list mechanics

### Irreversibility
- unchanged — additive migrations; Tier-4 retirements plan-only and now precondition-gated

### Known weak points (round 2)
- **Date-normalized trigger diff is the most novel untested mechanism** — jsonb day-array comparison with `date` stripped, in plpgsql; day reorder vs date change ambiguity (is reordering days a substantive edit? plan implies yes — verify the spec is implementable and the semantics defensible)
- **Fork-on-spec-edit UX/linkage** — fork changes the picker's winning row id; verify the (name, kit_size) dedupe claim against the actual picker merge code (`loadMaterialsCatalog` / Sales picker) and that Tab-3 `catalog_id` stamps the tenant fork, not the shadowed default
- **Grandfather boundary** — "absent = pre-feature" holds only if no code path writes `specs_confirmed` before Phase 2 ships; also Schedule's FieldSowBuilder (BF-11) adds materials WITHOUT the new stamp path until Phase 3 — those entries are absent-state = exempt; is that acceptable for Schedule-added materials between Phase 2 and 3?
- **Print gate is the only post-Send enforcement** — an unconfirmed spec can live on the SOW indefinitely and surface in Field's TasksTab (which has no gate); acceptable? (plan implies yes — crew ticket is the guarded artifact)
- **`updateJobWtcFieldSow` writes `start_date`/`end_date` besides `field_sow`** — trigger fires on UPDATE of the row; confirm the trigger predicate is on `field_sow` distinctness only, not span changes

### Open questions
- Count: 4 (§6, unchanged)
- Highest-pressure: #4 task-% home (Phase E boxed in by matrix) — unchanged from round 1

### Suggested attack angles (2 total)
1. **Verify the corrections** — re-walk each round-1 finding against 8864e98 + the cited source files; confirm the fix is faithful to code reality (esp. §0 rewrites, Phase-3 retrofit claim, §4.1 refutation — read `role_aware_money_rls.sql` yourself). Required reading: the revision diff (`git diff 5877539..8864e98`), ReportTab.js, queries.js:587-628, role_aware_money_rls.sql.
2. **Attack the new mechanisms** — the §-new-mechanisms list above, weak points as pressure map (trigger diff semantics, fork linkage, grandfather boundary, print-gate sufficiency). Required reading: §1/§2/§4 of the plan, WTCCalculator.jsx:477-483 + 730-745, CardSowModal.jsx/FieldSowBuilder, 20260518185000 trigger precedent.

### Suggested agent count: 2

Rationale: verification round over a corrected baseline — one agent confirms the fixes, one attacks only the added mechanisms; 3+ would re-audit unchanged material.
