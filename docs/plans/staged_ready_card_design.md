...
| 10 | ~~Cross-repo coordination with sales-command owner~~ — resolved: path (a) chosen. sales-command will ship a DROP migration for the legacy storage policy `"Authenticated can delete job-attachments"` in the same release window. sch-command stays the consumer | resolved (2026-05-28) |
| 11 | ~~search_path drift~~ — resolved: drop `pg_temp` from all new SECURITY DEFINER fns, align to `SET search_path = public` matching sales-command canonical helpers | resolved (2026-05-28) |
