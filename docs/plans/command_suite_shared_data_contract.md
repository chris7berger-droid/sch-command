...
| **Canonical location** | `job_wtcs[].field_sow` (per-WTC — multi-work-type jobs carry one SOW per WTC by construction). Proposal copy is frozen at Send; `jobs.field_sow` flat mirror is legacy → retire (Tier 4). |
